...
public class GuaraRobot extends Robot {
	...
	// private final PinJoint flexHipJoint;
	// private final PinJoint abdHipJoint;
	...
	lShankX = 0.04 / 2, lShankY = 0.06 / 3, lShankZ = 0.15, mShank = 3.6, IxxShank = inertiaMoment(lShankY, lShankZ),
			IyyShank = inertiaMoment(lShankX, lShankZ), IzzShank = inertiaMoment(lShankX, lShankY);
	...
	/*
	 * Theta will be the Knee joint angle rotation Psi will be the Ankle joint angle
	 * of rotation Phi will be the AbduHip joint Y axis angle of rotation lThighZ is
	 * l1 e lShankZ is l2 length
	 */
	...
	public GuaraRobot() {
		...
		// h = Math.sqrt(2 * lThighZ * lShankZ * Math.cos(theta) + Math.pow(lThighZ, 2)
		// + Math.pow(lShankZ, 2));

		// Setting Variables for guara`s posture;
		/*
		 * phiX and phiY will be the abdFlexHip joint anklePositionVector and Y axis
		 * angle of rotation thetaY will be the flexKnee joint rotation angle psiY will
		 * be the flexAnkle joint rotation angle lThighZ for l1 e lShankZ for l2
		 */
		...
		// knee and ankle
		abdFlexHip0 = new UniversalJoint("abdHip0", "flexHip0", new Vector3D(lRobot / 2, wRobot / 2, 0.0), this, Axis.X,
				Axis.Y);
		...
		// knee and ankle
		abdFlexHip1 = new UniversalJoint("abdHip1", "flexHip1", new Vector3D(-lRobot / 2, wRobot / 2, 0.0), this,
				Axis.X, Axis.Y);
		...
		// knee and ankle
		abdFlexHip2 = new UniversalJoint("abdHip2", "flexHip2", new Vector3D(-lRobot / 2, -wRobot / 2, 0.0), this,
				Axis.X, Axis.Y);
		...
		// knee and ankle
		abdFlexHip3 = new UniversalJoint("abdHip3", "flexHip3", new Vector3D(lRobot / 2, -wRobot / 2, 0.0), this,
				Axis.X, Axis.Y);
		...
		phiX = Math.asin(lShankZ * Math.sin(theta) / h); // PhiY will be the abdFlexHip joint X axis angle of rotation
		phiY = Math.PI / 12; // PhiX will be the abdFlexHip joint Y axis angle of rotation
		theta = -Math.PI / 4; // Theta will be the flexKnee joint rotation angle
		...
		phiX = Math.asin(lShankZ * Math.sin(thetacount) / h);
		((UniversalJoint) abdFlexHip0).setInitialState(0, 0, phiX, 0);
		((UniversalJoint) abdFlexHip1).setInitialState(0, 0, phiX, 0);
		((UniversalJoint) abdFlexHip2).setInitialState(0, 0, phiX, 0);
		((UniversalJoint) abdFlexHip3).setInitialState(0, 0, phiX, 0);
